        ...
        <div class="xl:col-span-2 space-y-6">
            ...
            <div class="admin-card overflow-hidden">
                ...
                <div class="p-6 space-y-5">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-700 dark:text-gray-300 uppercase mb-2">Slug</label>
                    ...
                    <div>
                        ...
                        <textarea name="description" class="admin-input w-full h-32" placeholder="VD: Mua kèm giảm sốc - Chỉ hôm nay!">{{ old('description', $combo->description) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="admin-card overflow-hidden">
                <div class="p-6 border-b border-[var(--admin-border)] bg-[var(--admin-surface-muted)] flex items-center gap-2">
                    ...
                    <h3 class="font-bold text-slate-800 dark:text-slate-200 uppercase text-sm">Sách Trong Combo</h3>
                </div>
                
                <div class="max-h-[400px] overflow-y-auto space-y-2 pr-2 custom-scrollbar p-6">
                    @foreach($books as $book)
                    <label class="flex items-center gap-3 p-3 rounded-xl border {{ in_array($book->id, $selectedBooks) ? 'border-primary bg-red-50/30' : 'border-gray-100 dark:border-slate-700' }} hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-all cursor-pointer group">
                ...
                <div class="p-6 space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" name="original_price" value="{{ old('original_price', $combo->original_price) }}"
                                class="admin-input w-full">
                        </div>
                        <div>
                            <label class="block text-[10px] font-bold text-gray-400 uppercase mb-1">Giá Combo</label>
                            <input type="number" name="sale_price" value="{{ old('sale_price', $combo->sale_price) }}"
                                class="admin-input w-full">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="sort_order" value="{{ old('sort_order', $combo->sort_order) }}"
                            class="admin-input w-full">
                    </div>
                    ...
                    <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-slate-700/30 rounded-xl border border-gray-100 dark:border-slate-700">
                        ...
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="is_visible" value="1" {{ $combo->is_visible ? 'checked' : '' }} class="sr-only peer">
                ...
                <div class="p-6 space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="color" name="bg_from" value="{{ old('bg_from', $combo->bg_from) }}"
                                class="w-full h-10 rounded-lg cursor-pointer">
                        </div>
                        <div>
                            <label class="block text-[10px] font-bold text-gray-400 uppercase mb-1">Màu nền đến</label>
                            <input type="color" name="bg_to" value="{{ old('bg_to', $combo->bg_to) }}"
                                class="w-full h-10 rounded-lg cursor-pointer">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" name="icon" value="{{ old('icon', $combo->icon) }}"
                            class="admin-input w-full" placeholder="VD: psychology">
                    </div>

                    <div>
                        <label class="block text-[10px] font-bold text-gray-400 uppercase mb-1">Ảnh Combo hiện tại</label>
                        @if($combo->image)
                            <div class="mt-2 mb-4 w-20 h-20 rounded-lg overflow-hidden border border-gray-200">
