...
/**
 * Tests database persistence behavior for admin entities.
 */
class DbPersistenceTest extends BaseAdminTestCase
{
    use AssertsCrud;
    ...
    public function test_create_increases_count_by_one_for_banner(): void
    {
        $this->assertCountIncreasedBy('banner', 1, function () {
            Banner::create([
                'title'      => 'New Banner',
                'position'   => 'home_main',
                'is_visible' => true,
    ...
    public function test_create_increases_count_by_one_for_coupon(): void
    {
        $this->assertCountIncreasedBy('coupons', 1, function () {
            Coupon::create([
                'code'   => 'PERSIST01',
                'name'   => 'Persist Coupon',
                'type'   => 'percentage',
    ...
    public function test_update_does_not_change_count_for_banner(): void
    {
    ...
    public function test_soft_delete_sets_deleted_at_for_banner(): void
    {
        $banner = Banner::create([
            'title'      => 'Delete Me',
            'position'   => 'home_main',
        ...
        $record = Banner::withTrashed()->find($id);
        $this->assertNotNull($record->deleted_at, 'deleted_at should be set after soft delete');
    }

    public function test_soft_delete_sets_deleted_at_for_book(): void
    {
        $book = Book::create([
            'sku'   => 'SOFT-DEL-001',
        ...
        $record = Book::withTrashed()->find($id);
        $this->assertNotNull($record->deleted_at, 'deleted_at should be set after soft delete');
    }

    public function test_updated_at_gte_created_at_after_update(): void
    {
        $banner = Banner::create([
            'title'      => 'Timestamp Banner',
            'position'   => 'home_main',
            'is_visible' => true,
            'sort_order' => 1,
        ]);
        ...
        $banner->update(['title' => 'Timestamp Banner Updated']);
        $banner->refresh();
    ...
    public function test_soft_deleted_record_count_including_trashed_unchanged(): void
    {
        $banner = Banner::create([
            'title'      => 'Trashed Banner',
            'position'   => 'home_main',
            'is_visible' => true,
            'sort_order' => 1,
        ]);

        $countBefore = Banner::withTrashed()->count();

        $banner->delete();

        $countAfter = Banner::withTrashed()->count();

        $this->assertEquals($countBefore, $countAfter, 'withTrashed() count should not change after soft delete');
    }
}
